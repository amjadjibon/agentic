"""Base rapper agent for rap battles"""
...
class BaseRapperAgent(ABC):
    """Base class for rapper agents in rap battles"""
    
    def __init__(self, model_name: str, rapper_name: str, icon: str, next_rapper: str = "next"):
        self.model_name = model_name
        self.rapper_name = rapper_name
        self.icon = icon
        self.next_rapper = next_rapper
        self.persona = self._get_base_persona() + "\n\n" + self._get_rapper_specific_persona()
    
    def _get_base_persona(self) -> str:
        """Get the base rap battle persona"""
        return """You are a legendary rapper participating in an intense rap battle. Your goal is to deliver devastating bars, clever wordplay, and lyrical superiority while maintaining your unique style and persona.

RAP BATTLE RULES:
1. DELIVER FIRE BARS: Create hard-hitting, clever, and memorable lines
2. USE WORDPLAY: Incorporate puns, double entendres, metaphors, and clever rhymes
3. SHOW LYRICAL SKILL: Demonstrate complex rhyme schemes and flow patterns
4. BE COMPETITIVE: This is a battle - be aggressive but creative
5. STAY IN CHARACTER: Maintain your unique rapper persona and style
6. RESPOND TO OPPONENT: Address their previous bars and counter-attack
7. USE RAP BATTLE FORMAT: Structure your verses like actual rap battle rounds
8. BE CREATIVE: Use storytelling, punchlines, and unexpected angles

VERSE STRUCTURE:
- Keep verses to 8-16 bars
- Use clear rhyme schemes
- Build to powerful punchlines
- Include quotable moments
- Show technical skill

BATTLE TACTICS:
- Counter opponent's strengths
- Exploit any weaknesses mentioned
- Use their style against them
- Create memorable moments
- Build momentum through your verse

Remember: This is about lyrical skill, creativity, and entertainment. Bring the heat!"""

    @abstractmethod
    def _get_rapper_specific_persona(self) -> str:
        """Get the specific persona for this rapper"""
        pass
    
    def get_persona_with_tools(self) -> str:
        """Get persona with tool descriptions if tools are enabled"""
        from debateai.tools import get_tool_descriptions
        tools_desc = get_tool_descriptions()
        return self.persona + f"\n\nAVAILABLE RESEARCH TOOLS:\n{tools_desc}\n\nUse tools to research current rap references, recent events, or cultural context when relevant."
    ...
    def stream_response(self, state: ChatState, with_tools: bool = False, ui: Optional['DebateUI'] = None) -> Iterator[ChatState]:
        """Generate streaming rap battle response"""
        try:
            ...
            yield {
                ...
                "current_speaker": self.next_rapper,
        ...
        if messages and isinstance(messages[0], HumanMessage):
            if len(messages) == 1:
                # First verse - add persona to the initial battle topic
                messages[0] = HumanMessage(
                    content=persona_text + "\n\n" + messages[0].content
                )
            else:
                # Subsequent verses - add persona as system context but keep all battle history
                system_context = HumanMessage(content=persona_text + "\n\nPlease respond to the ongoing rap battle by dropping your next verse and addressing the previous bars.")
                messages = [system_context] + messages
        
        # Display rapper header
        if ui:
            ui.console.print(f"\n{self.icon} [bold]{self.rapper_name} drops bars:[/bold]")
        else:
            print(f"\n{self.icon} {self.rapper_name}:")
        ...
        try:
            ...
            if ui:
                ...
                print(f"\n❌ {error_msg}")
            accumulated_content = f"Error occurred during verse generation: {str(e)}"
        ...
        if tool_calls and with_tools:
            ...
            for tool_call in tool_calls:
                ...
                if ui:
                    from debateai.rich_ui import DebateUIComponents
            ...
            if ui:
                ui.console.print(f"\n{self.icon} [bold]{self.rapper_name} (incorporating research):[/bold]")
            else:
                print(f"\n{self.icon} {self.rapper_name} (incorporating research):")
